...
const ConfirmationNode = ({ data, id, selected }) => {
  const { confirmationText = '', updateNodeData } = data;

  const handleTextChange = (e) => {
    if (updateNodeData) {
      updateNodeData(id, { ...data, confirmationText: e.target.value });
    }
  };
  ...
  return (
    <div className={`relative flex flex-col gap-3 p-4 rounded-lg bg-white dark:bg-slate-800 shadow-lg border-2 cursor-pointer z-10 w-64 ${selected ? 'border-primary' : 'border-slate-200 dark:border-slate-700'}`}>
      ...
      <div className="space-y-3 pt-3 border-t border-border-light dark:border-border-dark">
        <div>
          <label className="text-xs font-medium text-muted-light dark:text-muted-dark" htmlFor={`confirmation-text-${id}`}>Confirmation Text</label>
          <textarea
      ...
      <Handle
        ...
        id="confirmed"
        className="absolute -right-1.5 top-1/3 -translate-y-1/2 w-3 h-3 rounded-full bg-slate-400 border-2 border-white dark:border-slate-800"
      />
      <Handle
        type="source"
        position={Position.Right}
        id="denied"
        className="absolute -right-1.5 top-2/3 -translate-y-1/2 w-3 h-3 rounded-full bg-slate-400 border-2 border-white dark:border-slate-800"
      />
    </div>
  );
};
