...
const ConfirmationNode = ({ data, selected }) => {
  return (
    <div
    ...
    >
      ...
      <div className="space-y-3 pt-3 border-t border-border-light dark:border-border-dark">
        <div>
          <label className="text-xs font-medium text-muted-light dark:text-muted-dark" htmlFor="confirmation-text">Confirmation Text</label>
          <textarea
            ...
            rows="2"
            defaultValue={data.confirmationText}
            onChange={(e) => data.onChange({ ...data, confirmationText: e.target.value })}
          ></textarea>
        </div>
      </div>
      <Handle
