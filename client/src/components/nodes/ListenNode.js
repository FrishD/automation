...
const ListenNode = ({ data, id }) => {
  ...
  return (
    <div
      className="relative flex flex-col gap-3 p-4 rounded-lg bg-white dark:bg-slate-800 shadow-lg border-2 border-slate-200 dark:border-slate-700 z-0"
    ...
    >
      ...
      <div className="text-xs text-muted-light dark:text-muted-dark pt-3 border-t border-border-light dark:border-border-dark">
        ...
        <div className="grid grid-cols-2 gap-x-2 mt-2">
          ...
          <div className="flex flex-col gap-2">
            <label htmlFor={`retries-${id}`} className="font-medium text-xs">Retries</label>
            <input
              type="number"
              id={`retries-${id}`}
              name="retries"
        ...
        <div className="mt-4 p-2 rounded-lg bg-slate-50 dark:bg-slate-900/50 min-h-[50px]">
          <p className="text-center text-slate-400 text-xs italic">Drop variable nodes here</p>
        </div>
      </div>
      <Handle
        type="target"
